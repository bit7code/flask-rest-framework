...
class TestBasicAuthentication(BaseFuncTest):
    ...
    @mock.patch(
        "flask_restframework.authentication.request",
        headers={"Authorization": "basic wrong_base64_auth_token"},
    )
    ...
    def test_base64_token(self, mock_headers):
        ...
        mock_headers.headers = {
            "Authorization": "basic " + base64.b64encode(b"waro163:passwd123").decode("utf-8")
        }
        # for init user class
        rf = RestFramework()
...
class TestJWTAuthentication(BaseFuncTest):
    ...
    @mock.patch(
        "flask_restframework.authentication.request",
        headers={"Authorization": "bearer not_config_jwt_secret"},
    )
    ...
    @mock.patch(
        "flask_restframework.authentication.request", headers={"Authorization": "bearer wrong_jwt_auth_token"}
    )
